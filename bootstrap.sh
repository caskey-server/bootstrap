...
if [[ -f "${AGE_KEY_FILE}" ]]; then
    ...
    info "Setting up age key for secret decryption..."
    echo ""
    echo "  Paste your age private key (starts with AGE-SECRET-KEY-1)."
    echo "  This is required to decrypt secrets during deployment."
    echo ""

    read -rsp "age private key: " age_key
    echo
    ...
    chown -R "${SUDO_USER}:${SUDO_USER}" "${USER_HOME}/.config/sops"
